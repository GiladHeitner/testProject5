...
- 🎬 **Hook video PiP intro wired in 2026-07-01** (was dead code): `shorts_bot.py` fetches a Pexels clip via `fetch_best_hook_video` and passes it to `compose_video` — moving clip replaces the static opening popup (same 700px slot, first `HOOK_VIDEO_SECONDS`≈2.5s, 0 disables, falls back to static popup when fetch fails). Fixed the latent `[i+2:v]` popup input-index bug in `build_filter_complex` that corrupted popups whenever a hook input existed (likely why the feature was abandoned) — popups now use `popup_input_start`. **Render-verified via frame inspection** (harness at scratchpad `render_harness.py`, session-local). (`reddit_topics.py`): +6 priority for concrete-incident/injustice sources (`_INCIDENT_KEYWORDS`), −4 for opinion-musing titles ("Why do people think..."), and **out-of-season Ramadan sources hard-rejected** (`is_ramadan_season()`, windows table 2026–2030; script prompt gets a SEASON NOTE to retell any fasting mention as "last Ramadan").
...
**Known deferred items from the audit (not yet done):** popup entrance "pop" animation; caption scale-pop + drop random caps; LLM story-quality judge for topic pick; wire unused emoji-caption layer. TTS 1.7× speed is memory-protected (Adam clone) — verify production path before touching.
