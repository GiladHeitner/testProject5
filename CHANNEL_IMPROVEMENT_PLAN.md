...
- [ ] **Recurring identity.** Lean into "Omar" as a named, recognizable character across videos (consistent intro tag, consistent visual signature) so the channel is a _person to follow_, not a feed.
- [ ] **Series / open loops.** Add a "Part 1 / Part 2" mechanic for the strongest stories — generate genuine cliffhangers that resolve in a follow-up upload.
